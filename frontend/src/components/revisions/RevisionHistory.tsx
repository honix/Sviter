...
import { fetchRevisions } from '../../services/api';
...
export function RevisionHistory({ pageId, onRevisionSelect, onRestoreRevision }: RevisionHistoryProps) {
  ...
  const loadRevisions = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await fetchRevisions(pageId);
      setRevisions(data);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-muted-foreground">Loading revision history...</div>
      </div>
    );
  }
  ...
  if (revisions.length === 0) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-muted-foreground">No revisions yet</div>
      </div>
    );
  }
